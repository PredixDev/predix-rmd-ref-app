...
SIMULATION_FILE="predix-rmd-ref-app/scripts/rmd-simulation.json"
SCRIPT="-script build-basic-app.sh -script-readargs build-basic-app-readargs.sh"
QUICKSTART_ARGS="-pxclimin 0.6.18 -uaa -asset -ts -dx -sim -rmd $ASSET_MODEL -psrmd $SCRIPT -sim-file $SIMULATION_FILE"
...
VERSION_JSON_URL="$GITHUB_RAW/predix-rmd-ref-app/$BRANCH/version.json"
...
function check_internet() {
  ...
  curl "http://bing.com" > /dev/null 2>&1
  if [ $? -ne 0 ]; then
