...
print ('userabbitmq='+config.useDataexchangeRabbitMQ)
print ('useblobstore='+config.useDataexchangeBlobStore)
...
try:
	...
	if config.only not in (''):
		...
		if config.continueFrom in ('continue','deployReferenceAppCreateAssetAndTimeseries'):
			config.continueFrom = 'continue'
			refApp.deployReferenceAppCreateAssetAndTimeseries(config)
		#if config.continueFrom in ('continue','deployReferenceAppCreateBlobstore'):
		#	config.continueFrom = 'continue'
		#	refApp.deployReferenceAppCreateBlobstore(config)
	...
	print ('(Optional) Start the machine data simulator with this command: cf start ' + config.dataExchangeSimulatorAppName)
