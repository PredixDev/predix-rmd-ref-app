...
With the Base Reference App however, the Predix Machine is not installed by the install script.  Instead we install a microservice called the Data Exchange Simulator.  Data Flows from the Data Exchange Simulator to the Data Exchange and on to Predix Time Series.

We also install a custom Web Socket Server microservice which shows how data can flow real-time to the Reference App UI.  The data-exchange sends the same data that it receives from the simulator, which allows for a Live websocket feed to the RMD UI.
